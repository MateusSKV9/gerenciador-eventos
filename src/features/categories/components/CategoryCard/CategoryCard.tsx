...
export function CategoryCard({ id, name, color, isMenuOpen, toggleMenu, closeMenu, showModal }: CategoryCardProps) {
	...
	useEffect(() => {
		function handleClickOutside(e: MouseEvent) {
			if (isMenuOpen && menuRef.current && e.target instanceof Node && !menuRef.current.contains(e.target)) {
				closeMenu();
			}
		}
		...
		<div className={styles.card}>
			<div className={styles.wrapper}>
				...
				<button
					...
					title="Menu"
				>
					<svg width={35} className={styles.icon_menu} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640">
						<path
							fill="currentColor"
				...
				{isMenuOpen && <ItemMenu ref={menuRef} onEdit={handleEdit} onDelete={handleDelete} />}
			</div>
